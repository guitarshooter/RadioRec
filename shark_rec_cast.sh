...
#/usr/bin/ecasound -i /dev/dsp$DEVNO -o:stdout -t:$1 |/usr/local/bin/ffmpeg -i -acodec libmp3lame -ab 128 -ar 44100 $FILENAME.mp3 >>$ROOTDIR/log.txt 2>&1
/usr/local/bin/ffmpeg -f oss -i /dev/dsp$DEVNO -t $1 -acodec libmp3lame -ab 128 -ar 44100 $FILENAME.mp3 >>$ROOTDIR/log.txt 2>&1 
ENDTIME=`date +%H:%M`
sleep 10
...
#perl -w /root/bin/set_mp3tag.pl $FILENAME.mp3 TFM "Title5" "Titlel" "$4" "$5" $STARTTIME $ENDTIME >>$ROOTDIR/log.txt 2>&1
